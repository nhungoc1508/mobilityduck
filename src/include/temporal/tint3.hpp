...
struct TInt3 {
    std::vector<uint8_t> wkb_data;
    
    static LogicalType TInt3Make();
    ...
    static bool TemporalToBlob(Vector &source, Vector &result, idx_t count, CastParameters &parameters);
    static bool BlobToTemporal(Vector &source, Vector &result, idx_t count, CastParameters &parameters);
